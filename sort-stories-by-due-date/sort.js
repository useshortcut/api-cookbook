...
rp(getOptions).then( (searchResults) => { // Get stories using request-promise.
  ...
  // Send priority order updates to Clubhouse. Wait for each call to finish before going to the next one.
  async function sendPriorityOrderUpdates() {
    for (let i = 1; i < stories.length; i++) {
      putOptions.uri = 'https://api.clubhouse.io/api/v2/stories/' + stories[i].id            
